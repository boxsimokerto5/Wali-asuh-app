...
import { X, Save, User, MapPin, Calendar, Hash, Users, Shirt, Footprints, Plus } from 'lucide-react';
import { ChildData } from '../types';

interface ChildAddFormProps {
  onSave: (newData: ChildData) => void;
  onCancel: () => void;
  nextId: number;
}

export default function ChildAddForm({ onSave, onCancel, nextId }: ChildAddFormProps) {
    ...
    >
      <div className="bg-blue-600 p-6 text-white flex items-center justify-between">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h2 className="text-lg font-black tracking-tight">Tambah Siswa Baru</h2>
            <p className="text-[10px] uppercase tracking-widest text-blue-100 font-bold">Lengkapi data di bawah ini</p>
          </div>
        </div>
        <button 
          onClick={onCancel}
      ...
      <form onSubmit={handleSubmit} className="p-6 space-y-6 max-h-[70vh] overflow-y-auto custom-scrollbar">
        ...
        <div className="space-y-4">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
            <div>
              <label className={labelClasses}>Nomor ID (KK)</label>
              <input
                type="text"
                name="nomor"
